...
• Server prevents system sleep while running (stays active when PC locked)"""
        ...
        tk.Radiobutton(provider_options_frame, text="✨ OpenCode Zen (Cloud)",
                       variable=self.provider_var, value='opencode-zen',
                       bg=COLORS['bg_card'], fg=COLORS['text_primary'],
                       selectcolor=COLORS['bg_input'], activebackground=COLORS['bg_card'],
        ...
        def update_provider_ui(*args):
            ...
            if provider == 'nvidia':
                ...
                self.test_conn_btn.config(text="✨ Test OpenCode Zen Connection")
            else:
  ...
  • Get an OpenCode Zen API key from https://opencode.ai/zen
  ...
  • API key format: nvapi-..."""
        ...
        # OpenCode Zen API Key Card
        zen_card = ModernCard(scrollable_frame, "✨ OpenCode Zen Configuration")
        zen_card.pack(fill=tk.X, pady=(0, 15))
        
        zen_key_frame = tk.Frame(zen_card.content, bg=COLORS['bg_card'])
        zen_key_frame.pack(fill=tk.X, pady=15)
        
        tk.Label(zen_key_frame, text="OpenCode Zen API Key", font=('Segoe UI', 12, 'bold'),
                bg=COLORS['bg_card'], fg=COLORS['text_primary']).pack(anchor=tk.W)
        tk.Label(zen_key_frame, text="Get your API key from https://opencode.ai/zen",
                font=('Segoe UI', 10), bg=COLORS['bg_card'],
                fg=COLORS['text_secondary']).pack(anchor=tk.W, pady=(2, 10))
        
        self.zen_api_key_entry = tk.Entry(zen_key_frame, font=('Consolas', 12),
                                   bg=COLORS['bg_input'], fg=COLORS['text_primary'],
                                   insertbackground=COLORS['text_primary'],
                                   relief=tk.FLAT, bd=0, highlightthickness=0, width=30,
                                   show='*')
        self.zen_api_key_entry.pack(anchor=tk.W, fill=tk.X, ipady=8)
        self.zen_api_key_entry.insert(0, self.config.get('opencode_zen_api_key', ''))
        
        zen_help_text = """💡 OpenCode Zen is an AI gateway providing access to curated models:
  • Free tier: DeepSeek V4 Flash, MiniMax M2.5, Ring 2.6 1T, Nemotron 3 Super, Big Pickle
  • Paid models: GPT 5.x, Claude 4.x, Gemini 3.x, Qwen, Kimi, GLM
  • Endpoint: https://opencode.ai/zen/v1"""
        tk.Label(zen_card.content, text=zen_help_text, font=('Segoe UI', 10),
                bg=COLORS['bg_card'], fg=COLORS['text_secondary'],
                justify=tk.LEFT, wraplength=700).pack(anchor=tk.W, pady=(0, 10))
        ...
        chat_toggle_checkbox = tk.Checkbutton(chat_toggle_frame,
                                               text="Enable chat (requires LLM provider)",
    ...
    def setup_dependencies_page(self):
        ...
        tk.Label(info_card, text="i️ About", font=('Segoe UI', 12, 'bold'),
...
Status:
  ...
  ○ Gray = Not found"""
    ...
    def install_dependency(self, key):
        ...
        if key == 'node':
            self.dep_log("i️ Opening Node.js download page...")
            self.dep_log("   Please download and install Node.js from the website")
            webbrowser.open('https://nodejs.org/')

        elif key == 'npm':
            self.dep_log("i️ npm comes bundled with Node.js")
        ...
        def update():
            ...
            if tag is None:
                if any(kw in message.lower() for kw in ['✅', 'success', 'complete', 'installed']):
                    ...
                    tag = 'warning'
                elif any(kw in message.lower() for kw in ['i️', 'info', 'installing', 'checking']):
                    tag = 'info'
    ...
    def test_zen_connection(self):
        """Test OpenCode Zen connection"""
        api_key = self.zen_api_key_entry.get().strip()
        
        if not api_key:
            messagebox.showwarning("Warning", "Please enter your OpenCode Zen API key first.\n\nGet one from https://opencode.ai/zen")
            return
        
        self.log(f"🔍 Testing OpenCode Zen connection...")
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                self.log(f"✅ OpenCode Zen connection successful!")
                self.log(f"📋 Available models: {model_count}")
                
                # Show free tier models
                free_models = [m for m in models if m.get('free') or 'free' in m.get('id', '').lower()]
                if free_models:
                    self.log(f"🆓 Free tier models: {len(free_models)}")
                
                # Show first few model names
                model_names = [m.get('id', m.get('name', 'Unknown')) for m in models[:5]]
                messagebox.showinfo("Success", f"✅ OpenCode Zen is reachable!\n\nModels: {model_count} available\nFree tier: {len(free_models)}\n\nFirst few models:\n" + "\n".join(model_names))
        
        except urllib.error.HTTPError as e:
            error_msg = e.read().decode('utf-8') if e.fp else str(e)
            self.log(f"❌ OpenCode Zen connection failed: HTTP {e.code}")
            
            if e.code == 401:
                messagebox.showerror("Error", "❌ Authentication failed!\n\nCheck that your OpenCode Zen API key is valid.")
            else:
                messagebox.showerror("Error", f"❌ OpenCode Zen returned error {e.code}:\n\n{error_msg}")
        
        except urllib.error.URLError as e:
            error_msg = str(e.reason)
            self.log(f"❌ OpenCode Zen connection failed: {error_msg}")
            
            if "connection refused" in error_msg.lower():
                messagebox.showerror("Error", "❌ Connection refused!\n\nCannot reach OpenCode Zen servers.")
            elif "timeout" in error_msg.lower():
                messagebox.showerror("Error", "❌ Connection timed out!\n\nOpenCode Zen servers are not responding.")
            else:
                messagebox.showerror("Error", f"❌ Cannot reach OpenCode Zen:\n\n{error_msg}")
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.config['opencode_zen_api_key'] = self.zen_api_key_entry.get().strip()
            ...
            zen_key = self.config.get('opencode_zen_api_key', '')
...
# LLM Provider (ollama, nvidia, or opencode-zen)
...
# OpenCode Zen API key (for opencode-zen provider)
